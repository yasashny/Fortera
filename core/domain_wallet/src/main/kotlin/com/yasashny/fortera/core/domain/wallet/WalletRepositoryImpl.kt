...
internal class WalletRepositoryImpl(
    ...
    override suspend fun createWallet(name: String, seedPhrase: SeedPhrase): Wallet {
        val wallet = Wallet(
            ...
            name = name.ifBlank { "My Wallet" },
            address = deriveAddress(seedPhrase),
        )
    ...
    private fun deriveAddress(seedPhrase: SeedPhrase): String {
        val bytes = MessageDigest.getInstance("SHA-256")
            .digest(seedPhrase.toDisplayString().toByteArray(Charsets.UTF_8))
        return "0x" + bytes.take(20).joinToString("") { "%02x".format(it) }
    }
...
private fun WalletEntity.toWallet() = Wallet(id, name, address, createdAt)
private fun Wallet.toEntity() = WalletEntity(id, name, address, createdAt)
